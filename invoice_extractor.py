...
RULES:
1. Extract EVERY billable line item - each row is a separate item even if same name
2. SKIP section headers (like "Blood Bank", "Equipment/Instrument", "Consultation")
3. SKIP subtotals and grand totals (rows that just say "Total")
4. SKIP patient info, dates, bill numbers at top of page
5. item_name: Short name from bill (if spans 2 lines, combine them)
6. item_amount: Net/Total amount (rightmost amount column, after discounts)
7. item_rate: Gross/Rate/Unit price (if shown, usually second-to-last amount column)
8. item_quantity: Just the NUMBER from Qty column (ignore "No", "Nos", "Units" text)

PAGE TYPE:
- "Pharmacy" for medicines/drugs (tablets, syrups, injections)
- "Final Bill" for summary pages with category totals only
- "Bill Detail" for itemized services/procedures/equipment

IMPORTANT:
- Each row with amounts is a separate item, even if same name as another row
- Quantity like "1 No" or "4 Nos" should be extracted as just 1 or 4
- If there's both Gross and Net columns, use Net for item_amount, Gross for item_rate

Return compact JSON:
{"page_type":"Bill Detail","bill_items":[{"item_name":"Service Name","item_amount":100.00,"item_rate":100.00,"item_quantity":1}]}
...
RULES:
1. Extract EVERY billable line item - each row is a separate item even if same name repeats
2. SKIP section headers (Blood Bank, Equipment, Consultation, etc.)
3. SKIP subtotal/total rows
4. item_name: Short name (combine if spans 2 lines)
5. item_amount: Net amount (rightmost column)
6. item_rate: Gross/Rate (second-to-last amount column)
7. item_quantity: Just the NUMBER (ignore "No", "Nos", "Units")

PAGE TYPES:
- "Pharmacy" for medicines/drugs
- "Final Bill" for summary pages
- "Bill Detail" for itemized services

Return as JSON array with one object per page:
[{{"page_no":"{page_nums[0]}","page_type":"Bill Detail","bill_items":[{{"item_name":"X","item_amount":100,"item_rate":80,"item_quantity":1}}]}},{{"page_no":"{page_nums[1]}","page_type":"Bill Detail","bill_items":[...]}}]

CRITICAL: Return ONLY valid JSON array. No markdown. Extract EVERY row with amounts."""
    ...
    def deduplicate_across_pages(self, all_page_items: List[Dict]) -> List[Dict]:
        ...
        summary_keywords = ['consultation', 'drugs', 'investigations', 'medical consumable', 
                          ...
                          'laboratory', 'nursing', 'bed charges']
        ...
        for page in all_page_items:
            ...
            # Summary pages have very few items with generic category names
            generic_count = 0
            for item in items:
                name_lower = item.get('item_name', '').lower().strip()
                # Check if it's a generic category name (usually short)
                if any(kw == name_lower for kw in summary_keywords):
                    generic_count += 1
            
            # If most items are generic categories and few items total, it's a summary
            if len(items) <= 10 and generic_count >= len(items) * 0.3:
                summary_pages.append(page)
                page['page_type'] = 'Final Bill'
            else:
    ...
    def retry_extraction(self, image: Image.Image, page_num: int) -> Optional[Dict]:
        ...
        try:
            retry_prompt = """Extract ALL billable items from this invoice page as compact JSON.
Each row with amounts = separate item (even if same name repeats).
Skip headers and totals.
Quantity: extract just the number (ignore "No"/"Nos").
...
Return ONLY JSON."""
    ...
    def _parse_quantity(self, value) -> float:
        """Parse quantity, handling formats like '1 No', '4 Nos', '2 Units'"""
        if value is None:
            return 0.0
        if isinstance(value, (int, float)):
            return float(value)
        try:
            ...
            for suffix in [' No', ' Nos', ' Units', ' Unit', ' Pcs', ' Pc', 'No', 'Nos']:
                str_val = str_val.replace(suffix, '')
            # Extract number
